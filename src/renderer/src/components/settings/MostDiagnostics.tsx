...
import { MostDiagnosticDevice, useCarplayStore, useMostDiagnosticsStore, useMostOperatingModeStore } from '../../store/store'
...
export default function MostDiagnostics() {
  ...
  const headUnitMode = useMostOperatingModeStore((current) => current.headUnit)
  const appSettings = useCarplayStore((current) => current.settings)
  const saveAppSettings = useCarplayStore((current) => current.saveSettings)
  ...
  return <Box sx={{ height: '100%', p: 1, display: 'grid', gridTemplateRows: '46px minmax(0,1fr)', gap: 1 }}>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      ...
      {!headUnitMode && <Button size="small" variant="contained" disabled={!appSettings} onClick={() => appSettings && saveAppSettings({ ...appSettings, diagnosticMode: false })}>Exit diagnostics</Button>}
