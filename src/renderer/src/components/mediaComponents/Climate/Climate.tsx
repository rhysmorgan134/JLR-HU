...
export default function Climate() {
  ...
  const goBack = () => navigate(location.pathname.startsWith('/settings/') ? '/settings' : '/home')

  return <Box sx={{ height: '100%', p: 1.5, display: 'grid', gridTemplateRows: '46px minmax(0,1fr)', gap: 1.25, overflow: 'hidden' }}>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}><IconButton aria-label="Back" onClick={goBack} sx={{ width: 42, height: 42, color: 'white', background: 'rgba(255,255,255,.06)' }}><ArrowBackRoundedIcon /></IconButton><Box><Typography sx={{ fontSize: 11, color: 'primary.main', fontWeight: 700, letterSpacing: 1.7 }}>COMFORT</Typography><Typography sx={{ fontSize: 22, fontWeight: 500, lineHeight: 1 }}>Climate</Typography></Box><Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center', gap: 1 }}><Typography sx={{ fontSize: 11, color: 'text.secondary', letterSpacing: 1.2 }}>FAN</Typography>{Array.from({ length: 7 },(_,i)=><Box key={i} sx={{ width: 8, height: 12+i*3, borderRadius: 1, background: i < state.fanSpeed ? 'primary.main' : 'rgba(255,255,255,.1)' }} />)}</Box></Box>
