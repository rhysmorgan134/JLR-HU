...
function Base() {
  ...
  const sourceOverview = currentSource === 'AudioDiskPlayer'
    ? <AudioDiskOverview />
  ...
  return <Box sx={{ height: '100%', p: 1.5, display: 'grid', gridTemplateColumns: '58px minmax(0,1fr)', gap: 1.5, overflow: 'hidden' }}>
    <Box className="glass-panel" sx={{ borderRadius: 3, py: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <Box sx={{ display: 'grid', gap: 1 }}>
        <IconButton aria-label="Audio settings" onClick={() => navigate('/settings/audio')} sx={{ width: 42, height: 42, color: 'white', background: 'rgba(255,255,255,.06)' }}><MusicNoteRoundedIcon /></IconButton>
        ...
        <IconButton aria-label="Climate" onClick={() => navigate('/climate')} sx={{ width: 42, height: 42, color: 'text.secondary' }}><DirectionsCarRoundedIcon /></IconButton>
        <IconButton aria-label="Settings" onClick={() => navigate('/settings')} sx={{ width: 42, height: 42, color: 'text.secondary' }}><SettingsRoundedIcon /></IconButton>
      </Box>
    </Box>
    ...
    <Box sx={{ minWidth: 0, display: 'grid', gridTemplateRows: 'minmax(0,1fr) 142px', gap: 1.5 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.3fr) minmax(220px,.7fr)', gap: 1.5, minHeight: 0 }}>
        <Box className="glass-panel touch-card" onClick={openCurrentSource} sx={{ borderRadius: 3, overflow: 'hidden', cursor: 'pointer', position: 'relative' }}>{sourceOverview}<ChevronRightRoundedIcon sx={{ position: 'absolute', right: 12, top: 12, color: 'text.secondary' }} /></Box>
